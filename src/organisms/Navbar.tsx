    ...
    >
      <div className="container mx-auto flex items-center justify-between px-4 py-3">
        ...
        <div className="hidden md:flex flex-1 items-center justify-center gap-2">
          {navLinks.map(link => (
            <a key={link.name} href={link.href} className="px-4 py-2 rounded-lg font-medium text-white transition hover:bg-neutral-800 hover:text-white">
              {link.name}
            </a>
          ))}
        </div>
        ...
        <div className="hidden md:flex items-center gap-4">
          <Link href="/signin" className="px-4 py-2 rounded-lg bg-neutral-900 text-white border border-neutral-700 hover:bg-neutral-800 transition font-semibold">
            Sign In
          </Link>
          <Link href="/get-started" className="px-4 py-2 rounded-lg bg-lime-400 text-black font-bold hover:bg-lime-300 transition">
            Get Started
          </Link>
        </div>
        {/* Hamburger */}
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div className="md:hidden bg-black/90 backdrop-blur-sm absolute top-full left-0 w-full flex flex-col items-center gap-6 py-6 border-b border-neutral-800">
          {navLinks.map(link => (
